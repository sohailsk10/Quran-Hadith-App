...
class _CountryWiseReciterSelectorState
  ...
  Widget build(BuildContext context) {
    ...
    return recitersAsync.when(
      ...
      data: (reciters) {
        final list =
            reciters.isNotEmpty ? reciters : ReciterCountryMapper.getStandardReciters();
  ...
  Widget _buildWithReciterList(
    ...
    final filtered = reciters.where((r) {
      ...
      final matchesCountry = r.countryName.toLowerCase().contains(q);
      final matchesStyle = r.style.name.toLowerCase().contains(q);
      return matchesName || matchesCountry || matchesStyle;
    }).toList();

    // Group by country
    final Map<ReciterCountry, List<ReciterInfo>> grouped = {};
    for (final r in filtered) {
      final country = r.countryObj;
      grouped.putIfAbsent(country, () => []).add(r);
    }
                  ...
                  return Padding(
                    ...
                    child: FilterChip(
                      ...
                      onSelected: (_) => setState(() {
                        _selectedCountryFilter = isSelected ? null : c.name;
                      }),
                      selectedColor: theme.colorScheme.primaryContainer,
                      checkmarkColor: theme.colorScheme.onPrimaryContainer,
                    ),
        ...
        if (displayCountries.isEmpty)
          ...
          ...displayCountries.map((country) {
            ...
            final hasSelectedReciter = countryReciters.any((r) =>
                ...
                r.apiId.toString() == currentSelectedId ||
                (currentSelectedId == 'ar.alafasy' && r.id == '7') ||
                (currentSelectedId == 'ar.abdulbasit' && r.id == '2') ||
                (currentSelectedId == 'ar.sudais' && r.id == '3') ||
                (currentSelectedId == 'ar.husary' && r.id == '6') ||
                (currentSelectedId == 'ar.minshawi' && r.id == '9'));
            ...
            return Container(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(AppConstants.radiusMD),
                child: ExpansionTile(
                  key: PageStorageKey('reciter_country_${country.name}'),
                  ...
                  children: countryReciters.map((reciter) {
                    final isSelected = reciter.id == currentSelectedId ||
                        reciter.apiId.toString() == currentSelectedId ||
                        (currentSelectedId == 'ar.alafasy' &&
                            reciter.id == '7') ||
                        (currentSelectedId == 'ar.abdulbasit' &&
                            reciter.id == '2') ||
                        (currentSelectedId == 'ar.sudais' &&
                            reciter.id == '3') ||
                        (currentSelectedId == 'ar.husary' &&
                            reciter.id == '6') ||
                        (currentSelectedId == 'ar.minshawi' &&
                            reciter.id == '9');

                    return InkWell(
                      onTap: () {
                        // Switch active reciter in audio service & settings
                        ref.read(quranAudioProvider.notifier).changeReciter(
                        ...
                        child: Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                reciter.style.name.toUpperCase(),
                                style: theme.textTheme.labelSmall?.copyWith(
                                  fontWeight: FontWeight.w600,
                                  fontSize: 10,
                                  color: theme.colorScheme.onSurfaceVariant,
